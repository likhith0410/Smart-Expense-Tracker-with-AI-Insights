...
const removeDebugToolbar = () => {
  // List of possible debug toolbar selectors
  ...
  debugSelectors.forEach(selector => {
    ...
    elements.forEach(element => {
      if (element) {
        element.remove();
        console.log(`🚫 Removed debug element: ${selector}`);
      }
    });
  });

  // Also hide with CSS
  const existingStyle = document.getElementById('debug-toolbar-killer');
  if (!existingStyle) {
    const style = document.createElement('style');
    style.id = 'debug-toolbar-killer';
    style.textContent = `
      #djDebug,
      .djDebug,
      #djDebugToolbar,
      ...
      /* Also hide any iframe that might contain debug content */
      iframe[src*="debug"],
      iframe[src*="djdt"] {
        display: none !important;
      }
...
class PWAManager {
  ...
  init() {
    ...
    // Remove debug toolbar immediately
    this.removeDebugToolbar();
  }

  // Remove debug toolbar method
  removeDebugToolbar() {
    removeDebugToolbar();
    
    // Keep checking every 500ms for the first 10 seconds
    let attempts = 0;
    ...
    const intervalId = setInterval(() => {
      ...
      if (attempts >= maxAttempts) {
        ...
        console.log('🚫 Debug toolbar removal completed');
      }
    }, 500);
  }
...
// Run after DOM loads
document.addEventListener('DOMContentLoaded', removeDebugToolbar);

// Run after everything loads
window.addEventListener('load', removeDebugToolbar);
...
window.addEventListener('load', () => {
  ...
  // Final debug toolbar removal
  removeDebugToolbar();
});

console.log('🚀 Smart Expense Tracker PWA initialized successfully!');
console.log('🚫 Debug toolbar removal script loaded');